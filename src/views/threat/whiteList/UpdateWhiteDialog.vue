<template>
    <el-dialog v-model="isVisible" :title="T2('updateWhiteTitle')" :width="950" :before-close="handleClose"
        ...
        <el-space :size="20" direction="vertical" alignment="flex-start" style="margin-top: 20px;width: 100%;"
            ...
            <el-form :label-width="100" style="font-size: 16px;" label-position="left">
                <el-form-item :required="true" :label="T2('flowId') + ':'">{{ props.flowId }}</el-form-item>
                <el-form-item :required="true" :label="T('domain')+':'">{{ props.domain }}</el-form-item>
                <el-form-item :required="true" :label="T('flowTitle')+':'">{{ title }}</el-form-item>
                <el-form-item v-for="(rule, idx) in addReqRules" :label="T('ruleIndex', [idx + 1])" style="width:100%"
                    :required="true">
...
// form
const addReqRules = ref<Array<UpdateThreatWhitelistReq_ruleInfo>>([]);
const remark = ref('');

watchEffect(() => {
    remark.value = props.remark;
    addReqRules.value = props.rules;
});
...
const initAddReqRules = () => {
    ...
    .then(data => {
        candicateFields.value = data.fields.map(targetName => ({ label: targetName, value: targetName }));
    })
