...
export const listThreatRuleOptions = async (): Promise<OptionType[]> => {
	const req: GetThreatNamesReq = {
		ruleId: '', // Empty string returns all rule_id to title mappings
	};

	try {
		const response = await api.getThreatNames(req).then((resp) => resp.response);
		// Convert the map to OptionType array
		return Object.entries(response.names).map(([ruleId, title]) => ({
			label: title,
			value: ruleId,
		}));
	} catch (err) {
