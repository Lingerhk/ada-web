<template>
    <el-drawer :size="1276" v-model="state.open" :close-on-click-modal="true">
        <template #header>
            <h3>{{ $t('message.risk.baseline.addTitle') }}</h3>
        </template>
        <template #default>
            <!-- description -->
            <div style="padding: 20px;">
                <el-row class="desc-value">{{ $t('message.risk.baseline.addDesc') }}</el-row>
                ...
                <el-row><el-button type="primary" size="default" @click="addDomain" :disabled="state.plans.length === state.domainOptions.length">{{ $t('message.risk.baseline.addDomain') }}</el-button></el-row>
            </div>
        </template>
        <template #footer>
...
import { listBaselineTmpl, listDomainOptions } from '/@/api/grpc/method';
...
const submit = () => {
    ...
    .then(data => {
        const ok = alertResult(data.result, t('message.risk.baseline.addSucc'), t('message.risk.baseline.addFail'));
        if (ok) {
            state.open = false;
        }
    })
...
const open = async () => {
    state.open = true;
    state.domainOptions = await listDomainOptions(); // .then(data => state.domainOptions = data);
    state.tmplOptions = (await listBaselineTmpl()).map(t => ({
        value: t.iD,
        label: t.name,
    } as OptionType));

    // state.plans = [[state.domainOptions[0], state.tmplOptions[0]]];
