<template>
    <el-drawer :size="1276" v-model="state.open" :close-on-click-modal="true">
        <template #header>
            <h3>{{ $t('message.risk.weakpwd.addTitle') }}</h3>
        </template>
        <template #default>
            <!-- description -->
            <div style="padding: 20px;">
                <el-row class="desc-value">{{ $t('message.risk.weakpwd.addDesc') }}</el-row>
                ...
                <el-row><el-button type="primary" size="default" @click="addDomain" :disabled="state.plans.length === state.domainOptions.length">{{ $t('message.risk.weakpwd.addDomain') }}</el-button></el-row>
            </div>
        </template>
        <template #footer>
...
import { listDomainOptions, listWeakpwdTmpl } from '/@/api/grpc/method';
...
const submit = () => {
    ...
    .then(data => {
        const ok = alertResult(data.result, t('message.risk.weakpwd.addSucc'), t('message.risk.weakpwd.addFail'));
        if (ok) {
            state.open = false;
        }
    })
...
const open = async () => {
    ...
    state.tmplOptions = (await listWeakpwdTmpl()).map(t => ({
        value: t.iD,
        label: t.name,
    } as OptionType));
